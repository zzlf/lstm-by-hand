...
class LSTM(nn.Module):
    def __init__(self, input_sz: int, hidden_sz: int):
        ...
        # i_t
        self.W_i1 = nn.Linear(input_sz, hidden_sz,False)
        self.W_i2 = nn.Linear(hidden_sz, hidden_sz,False)
        self.b_i  = nn.Parameter(torch.ones([hidden_sz]))

        # f_t
        self.W_f1 = nn.Linear(input_sz, hidden_sz,False)
        self.W_f2 = nn.Linear(hidden_sz, hidden_sz,False)
        self.b_f = nn.Parameter(torch.ones([hidden_sz]))

        # c_t
        self.W_c1 = nn.Linear(input_sz, hidden_sz,False)
        self.W_c2 = nn.Linear(hidden_sz, hidden_sz,False)
        self.b_c = nn.Parameter(torch.ones([hidden_sz]))

        # o_t
        self.W_o1 = nn.Linear(input_sz, hidden_sz,False)
        self.W_o2 = nn.Linear(hidden_sz, hidden_sz,False)
        self.b_o = nn.Parameter(torch.ones([hidden_sz]))
    ...
    def forward(self, x):
        ...
        h_t, c_t = (
                torch.zeros(b_s, self.hidden_size).to(device),
                torch.zeros(b_s, self.hidden_size).to(device),
        )
        ...
        for t in range(n_st):
            ...
            i_t = torch.sigmoid(self.W_i1(x_t) +  self.W_i2(h_t) + self.b_i)
            f_t = torch.sigmoid( self.W_f1(x_t) +  self.W_f2(h_t) + self.b_f)
            g_t = torch.tanh( self.W_c1(x_t) + self.W_c2(h_t)+ self.b_c)
            o_t = torch.sigmoid( self.W_o1(x_t) + self.W_o2(h_t) + self.b_o)
            c_t = f_t * c_t + i_t * g_t
...
class TextLSTM(nn.Module):
    def __init__(self):
        ...
        self.LSTM = LSTM(emb_size, n_hidden)
